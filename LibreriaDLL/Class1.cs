...
namespace LibreriaDLL
{
    public class Biblioteca
    {
        ...
        {
            SqlConnection conexion = new SqlConnection("Data Source=.;Initial Catalog=SVSOFTWARE;Integrated Security=True");
            conexion.Open();
            ...
            foreach(Control Campo in ObjetoError.Controls )
            {
                if (Campo is ErrorTxtBox)
                {
                    ...
                    if (objeto.ValidarNumeros == true)
                    {
                        ...
                        {
                            SiError = true;
                            ErrorProvider.SetError(objeto,"Solo se aceptan numero!");
                        }
                    }
                    if (objeto.ValidarLetras == true)
                    {
                        int contador = 0, EncontrarLetras = 0;
                        foreach (char Numero in objeto.Text.Trim())
                        {
                            if (char.IsNumber(objeto.Text.Trim(), contador))
                            {
                                EncontrarLetras++;
                            }
                            contador++;
                        ...
                        {
                            SiError = true;
                            ErrorProvider.SetError(objeto, "Solo se aceptan letras!");
                        }
                    }
                    else
                    {
